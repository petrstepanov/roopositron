...
#include "PsThermPdf.h"

PsThermPdf::PsThermPdf(const char *name, const char *title,
    RooAbsReal& _t,
    RooAbsReal& _lambdaJ,
...
PsThermPdf::PsThermPdf(const PsThermPdf& other, const char* name) :
...
Double_t PsThermPdf::evaluate() const {   
    if (t < 0) return 0.;
...
Double_t PsThermPdf::Power(Double_t a, Double_t b) const{
    return TMath::Power(a, b);
}

// Indefinite integral
Double_t PsThermPdf::indefiniteIntegral(Double_t y) const {
    Double_t lj = lambdaJ;
...
Int_t PsThermPdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const {
   if (matchArgs(allVars,analVars,t)) return 1;
   return 0;
}

// Analytical integral
Double_t PsThermPdf::analyticalIntegral(Int_t code, const char* rangeName) const {
    assert(code == 1);
    ...
    if (code==1){
        ...
        std::cout << "Error in PsThermPdf::analyticalIntegral" << std::endl;
    }
    return ret;
}